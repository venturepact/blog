<?php
/*
 Plugin Name: MailChimp List Subscriber Plugin
	...
	*/
	function __construct() {
		...
		parent::WP_Widget(  'nmedia_mail_chimp',
				'MailChimp',
				array('description' => 'MailChimp Form Widget'));



		/* hook activating plugin */
		register_activation_hook(__FILE__, array($this,'nmmc_install'));
		
		
		// activate textdomain for translations
		add_action('plugins_loaded', array($this, 'nm_mc_textdomain'));
		
		
		/* book: register widget when loading the WP core */
		add_action('widgets_init',  array($this, 'nm_mc_register_widgets'));
		
		/* hook: loading js stuff */
		add_action('wp_enqueue_scripts', array($this, 'load_js'));
		
		/* hook: styilng */
		add_action('wp_print_styles', array($this, 'add_nm_stylesheet'));
		
		
		/* hook deactivating plugin */
		register_deactivation_hook(__FILE__, array($this, 'nm_mc_unistall'));
		
		/*shortcode introduced in version 3.0*/
		add_shortcode( 'nm-mc-form', array($this, 'renderForm'));
	...
	*/
	function add_nm_stylesheet() {
		...
		if ( file_exists($myStyleFile) ) {
			...
			wp_add_inline_style( 'nm_mailchimp_stylesheet', get_option('nm_mc_custom_css') );		
	 	}
	}
