...
$plugin_name = "MailChimp List Subscriber Plugin";
$shortname = "nm_mc";
...
$nm_mc_options = array (
		...
		array( "name" => $plugin_name." Options","type" => "title"),

		array( "name" => __("General Settings", "nm_mailchimp_plugin"),"type" => "section"),	
		array( "type" => "open"),
		...
		array( 	"name" => __("Thanks Message", "nm_mailchimp_plugin"),
				...
				"type" => "textarea",
				"std" => "Thank you for subscribing to our newsletter!"),	
    ...
    if ( @$_GET['page'] == $option_page_name ) {
        ...
        if ( !empty($_REQUEST['action']) && 'save' == $_REQUEST['action'] ) {
				...
				foreach ($nm_mc_options as $value) {
                    ...
                    if( !empty( $_REQUEST[ $value['id'] ] ) ) { update_option( $value['id'], $_REQUEST[ $value['id'] ]  ); } else { delete_option( $value['id'] ); } 
                }
...
function nm_mc_admin() {
    ...
    if ( !empty($_REQUEST['saved']) ) echo '<div id="message" class="updated fade"><p><strong>'.$plugin_name.' '.__('Settings saved.','nm_mailchimp_plugin').'</strong></p></div>';
    if ( !empty($_REQUEST['reset']) ) echo '<div id="message" class="updated fade"><p><strong>'.$plugin_name.' '.__('Settings reset.','nm_mailchimp_plugin').'</strong></p></div>';
    if ( !empty($_REQUEST['reset_widgets']) ) echo '<div id="message" class="updated fade"><p><strong>'.$plugin_name.' '.__('Widgets reset.','nm_mailchimp_plugin').'</strong></p></div>';
...
<div class="rm_input rm_text">
	<label for="<?php echo $value['id']; ?>"><?php _e($value['name'], 'nm_mailchimp_plugin') ?></label>
 	<input name="<?php echo $value['id']; ?>" id="<?php echo $value['id']; ?>" type="<?php echo $value['type']; ?>" value="<?php if ( get_option( $value['id'] ) != "") { echo stripslashes(get_option( $value['id'])  ); } else { echo !empty($value['std']) ? $value['std'] : ''; } ?>" />
...
<div class="rm_input rm_textarea">
	<label for="<?php echo $value['id']; ?>"><?php _e($value['name'], 'nm_mailchimp_plugin') ?></label>
 	<textarea name="<?php echo $value['id']; ?>" type="<?php echo $value['type']; ?>" cols="" rows=""><?php if ( get_option( $value['id'] ) != "") { echo stripslashes(get_option( $value['id']) ); } else { echo !empty($value['std']) ? $value['std'] : ''; } ?></textarea>
 <small><?php _e($value['desc'], 'nm_mailchimp_plugin') ?></small><div class="clearfix"></div>
