...
define( 'OPTIN_MONSTER_AFTER_POST_PLUGIN_VERSION', '2.0.2' );
...
function optin_monster_after_post_filter_optin_themes( $themes, $type ) {
    ...
    $themes = array(
        ...
        'transparent' => array(
            'name'  => __( 'Transparent Theme', 'optin-monster-post' ),
            'image' => plugins_url( 'includes/themes/transparent/images/icon.png', __FILE__ ),
            'file'  => __FILE__
        ),
    );
...
function optin_monster_after_post_theme_api( $api, $theme, $optin_id, $type ) {
    ...
    switch ( $theme ) {
        ...
        case 'postal' :
            ...
            break;
        case 'transparent' :
            if ( ! class_exists( 'Optin_Monster_Post_Theme_Transparent' ) ) {
                require plugin_dir_path( __FILE__ ) . 'includes/themes/transparent/transparent.php';
            }
            $api = new Optin_Monster_Post_Theme_Transparent( $optin_id );
            break;
    }
