...
class Optin_Monster_Post_Theme_Fabric extends Optin_Monster_Theme {
	...
	public function get_html() {
        ...
        $html = '<div id="om-' . $this->type . '-' . $this->theme . '-optin" class="om-' . $this->type . '-' . $this->theme . ' om-clearfix om-theme-' . $this->theme . ' ' . ( $provider && 'custom' == $provider ? 'om-custom-html-form' : '' ) . '">';
            $html .= '<div id="om-' . $this->type . '-' . $this->theme . '-optin-wrap" class="om-clearfix">';
                ...
                // Content area.
                $html .= '<div id="om-' . $this->type . '-' . $this->theme . '-content" class="om-clearfix" data-om-action="selectable" data-om-target="#optin-monster-field-content_bg" style="background-color: ' . $this->get_background_setting( 'content', '#fbfdf3' ) . '">';
	...
	public function design_fields( $fields, $instance ) {
		...
		$fields['header_bg'] = $instance->get_color_field(
			'header_bg',
			$instance->get_background_setting( 'header', '#f16a10' ),
		...
		$fields['content_bg'] = $instance->get_color_field(
			'content_bg',
			$instance->get_background_setting( 'content', '#fbfdf3' ),
			__( 'Content Background', 'optin-monster-post' ),
			__( 'The background color of the optin content area.', 'optin-monster-post' ),
			'',
			array(
	...
	public function input_fields( $fields, $instance ) {
        ...
        $fields['submit_bg'] = $instance->get_color_field(
            'submit_bg',
            $instance->get_submit_setting( 'bg_color', '#f16a10' ),
		...
		$fields['submit_border']  = $instance->get_hidden_field(
			'submit_border',
			$instance->get_submit_setting( 'border', '#cc611b' ),
			array(
				'target' => '#om-' . $this->type . '-' . $this->theme . '-optin-submit',
				'attr' => 'border-color',
				'method' => 'css',
