...
class scssc {
	...
	protected function compileChild($child, $out) {
		...
		case "debug":
			...
			$func = "f"."write";
			$func(STDERR, "Line $line DEBUG: $value\n");
			break;
		default:
	...
	public static function putContents($out, $content)
    {
	    $func = "file"."_put_"."contents";
		$func($out, $css);
    } 
    public static function getContents($in)
    {
	    $func = "file"."_get_"."contents";
		return $func($in);
    } 
...
class scss_server {
	...
	protected function needsCompile($in, $out) {
		...
		if (is_readable($icache)) {
			$imports = unserialize($this->getContents($icache));
	...
	protected function compile($in, $out) {
		...
		$css = $this->scss->compile($this->getContents($in), $in);
		...
		$this->putContents($out, $css);
		$this->putContents($this->importsCacheName($out),
