...
Class ajax_login_register_Login Extends AjaxLogin {
    ...
    public function facebook_login(){
        ...
        $user = array(
            'username'   => intval($_POST['fb_response']['id']),
            'user_login' => intval($_POST['fb_response']['id']),
            'first_name' => sanitize_text_field($_POST['fb_response']['first_name']),
            'last_name'  => sanitize_text_field($_POST['fb_response']['last_name']),
            'user_email' => sanitize_text_field($_POST['fb_response']['email']),
            'user_url'   => sanitize_text_field($_POST['fb_response']['link']),
            'user_avatar'   => !empty($_POST['fb_response']['picture']["data"]["url"]) ? sanitize_text_field($_POST['fb_response']['picture']["data"]["url"]) : '',
            );
        ...
        if ( empty( $user['username'] ) ){
            ...
            if ( $user_obj == false ){
                ...
                $args = array(
			        'ID'  => $user_obj->ID,
			        'user_nicename' => $user_obj->first_name . '-' . $user_obj->last_name
			    );
			    wp_update_user( $args );
            }
            ...
            if(is_array($password) && !empty($password[0])) {
	            $password = $password[0];
            }
