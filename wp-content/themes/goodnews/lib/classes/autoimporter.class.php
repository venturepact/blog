...
if ( ! class_exists( 'WXR_Parser' ) ) {
	...
	require dirname( __FILE__ ) . '/parsers.class.php';
}
...
class Auto_Importer extends WP_Importer
{
	...
	var $processed_local_menu_ids = array();
	var $menu_item_orphans = array();
	var $missing_menu_items = array();

	var $fetch_attachments = false;
	var $attachment_urls = array();
	var $url_remap = array();
	...
	function process_attachment( $post, $url ) {
		...
		if(!empty($this->attachment_urls[md5($url)])) {
			return $this->attachment_urls[md5($url)];
		}
		...
		$this->attachment_urls[md5($url)] = $post_id;
